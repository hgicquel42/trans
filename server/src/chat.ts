...
export class ChatController {
	...
	async getUserFromReq(req: Request) {
		const token = req.headers.cookie.split('; ').find(cookie => cookie.startsWith('Authentication')).split('=')[1]
	...
	async handleConnection(socket: WebSocket, req: Request) {
		const user: User = await this.getUserFromReq(req)
	...
	updateChannels() {
		for (const client of this.clients.values()) {
			const cli = this.clients.get(client.socket)
	...
	onchannels(socket: WebSocket, data: {}) {
		...
		// }).map(([name, channel]) => name);
		// socket.send(msg("channels", channels))
		// console.log(client.name + " -> " + channels)
		this.updateChannels()
	}
