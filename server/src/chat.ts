...
export class ChatController {
  ...
  sendToAll(channel: Channel, data: any, from?: Client) {
    for (const client of channel.clients.values())
      if (!from || !client.blockeds.has(from)) {
        ...
        else if (channel.password != "" && channel.passwordEntered.has(client))
          client.socket.send(data)
      }
  }
  ...
  onmsg(socket: WebSocket, data: {
    ...
    if (target.blockeds.has(client))
      throw new Error(client.user.username + " is trying to send a private message to someone that blocked him.")
